...
def load_colmap_data_json(realdir):
    ...
    for k in imdata:
        ...
        imname = im.name
        posesdict[imname] = {}
        posesdict[imname]['pose'] = m.tolist()
        posesdict[imname]['f'] = f
        posesdict[imname]['w'] = w
        posesdict[imname]['h'] = h
    
    return posesdict

def save_poses_json(realdir, posesdict):
    jsonfile = os.path.join(realdir, 'poses.json')
    with open(jsonfile, "w", encoding='utf-8') as f:
        f.write(json.dumps(posesdict))

def load_data_json(realdir):
    cam_dict_file = os.path.join(realdir, 'poses.json')
    with open(cam_dict_file) as fp:
        cam_dict = json.load(fp)
        for x in sorted(cam_dict.keys()):
            pose = np.array(cam_dict[x]['pose']).reshape((4, 4))
            cam_dict[x]['pose'] = pose
    return cam_dict
...
def gen_poses_json(base, dir2, match_type, factors=None, is_run_colmap=True, **kwargs):
    # files_needed = ['{}.bin'.format(f) for f in ['cameras', 'images', 'points3D']]
    files_needed = ['poses.json']
    # if os.path.exists(os.path.join(basedir, 'sparse/0')):
    #     files_had = os.listdir(os.path.join(basedir, 'sparse/0'))
    # else:
    #     files_had = []
    ...
    if is_run_colmap:
        ...
        files_had = os.listdir(basedir)
        if True:#not any([f in files_had for f in files_needed]):
            print( 'Need to run COLMAP' )
            run_colmap(base, dir2, match_type, **kwargs)
        else:
            print('Don\'t need to run COLMAP')
            return
        
    print( 'Post-colmap')
    posesdict = load_colmap_data_json(basedir)

    save_poses_json(basedir, posesdict)

    move_args = [
        'mv', os.path.join(basedir, 'dense', 'meshed-poisson.ply'), basedir
    ]
    subprocess.run(move_args)

    # clear_args = [
    #     'rm', os.path.join(basedir, 'dense'), os.path.join(basedir, 'sparse'), os.path.join(basedir, 'colmap_output.txt'), '-rf'
    # ]
